    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        if (authHeader != null && authHeader.startsWith(this.tokenHead)) {
            ...
            if (username == null && !StringUtils.isEmpty(authToken)) {
                ...
                response.getWriter().println(gson.toJson(JsonResponse.error(ResponseCode.CODE_TOKEN_ERROR, "token失效,请重新登录")));
            ...
            if (StringUtils.isEmpty(cacheService.getTokenById(userDetail.getAdminId().toString())) || !cacheService.getTokenById(userDetail.getAdminId().toString()).equals(authToken)) {
                ...
                response.setCharacterEncoding("UTF-8");
                response.setContentType("application/json");
                response.getWriter().println(gson.toJson(JsonResponse.error(ResponseCode.CODE_NO_LOGIN, "token失效,请重新登录")));
                response.getWriter().flush();
                return;
            }
